...
void setup() {
    ...
    Serial.println("=== 三轴坐标控制程序 ===");
    Serial.println("输入格式1 (步进电机和舵机): X,Y,Z,servo1,servo2,servo3");
    Serial.println("例如: 5.0,3.0,0.8,180,90,90");
    Serial.println("坐标范围: X[0-9.5], Y[0-9], Z[0-1.5]");
    Serial.println("舵机范围: servo1[0-360], servo2[0-165], servo3[0-180]");
    Serial.println("单位: 圈数");
    Serial.print("当前坐标: X=");
    ...
    Serial.println("特殊命令: 'read' - 执行读取操作（舵机2到90度，Z轴到1圈）");
    Serial.println("=======================");
}
...
void loop() {
    ...
    if (Serial.available() > 0) {
        ...
        if (commaCount == 5) {
            ...
            if (firstCommaIndex != -1 && secondCommaIndex != -1 &&
                thirdCommaIndex != -1 && fourthCommaIndex != -1 && fifthCommaIndex != -1) {
                ...
                String servo3Str = input.substring(fifthCommaIndex + 1); // 改为直接读取角度值
                ...
                Serial.print(", s3=");
                Serial.println(servo3.targetAngle);



            } else {
                Serial.println("错误: 输入格式不正确，请使用 'X,Y,Z,servo1,servo2,servo3' 格式");
                Serial.println("例如: 5.0,3.0,0.8,180,90,90");
            }
        }
            ...
            Serial.println("1. 步进电机和舵机控制: X,Y,Z,servo1,servo2,servo3 (6个参数)");
            Serial.println("2. 继电器和PWM控制: <relay1>,<relay_select>,<pwm_value> (3个参数)");
            Serial.println("3. 特殊命令: 'read' - 执行读取操作");
        }
    }
    ...
    updateServo(servo1, channel_1, 360);
    updateServo(servo2, channel_2, 165);
    updateServo(servo3, channel_3, 180); // 改为180度最大角度
...
void executeReadCommand() {
    ...
    updateServo(servo2, channel_2, 165);//机械臂倾斜，放置后续移动撞翻花盆
    Serial.println("读取命令执行完成");
}
